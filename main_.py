...
st.markdown(
    ...
    <div>
        <h1>Cálculo do FPR - Arquivo de Posição</h1>
    </div>
...
if uploaded_zip is not None:
    ...
    if valores.size > 0:
            ...
            if "opção_selecionada" not in st.session_state:
                st.session_state["opção_selecionada"] = valor_inicial
            if "data_mes" not in st.session_state:
                st.session_state["data_mes"] = ""
            ...
            with col_fund1:
                opção_selecionada = st.selectbox(
                    'Selecione um fundo:',
                    valores,
                    index=list(valores).index(st.session_state["opção_selecionada"]) if st.session_state["opção_selecionada"] in valores else 0
                )
                st.session_state["opção_selecionada"] = opção_selecionada

            with col_fund2:
                data_mes = st.text_input(
                    'Informe a data desejada:',
                    value=st.session_state["data_mes"],
                    placeholder="Exemplo: 31/12/2024"
                )
                st.session_state["data_mes"] = data_mes
                
            if "cota" not in st.session_state:
                st.session_state["cota"] = "SUB"
            if "investimento" not in st.session_state:
                st.session_state["investimento"] = 0.0

            
            with col_fund3:
                cota = st.selectbox(
                    "Selecione o tipo de cota:", 
                    options=['SUB', 'SR'], 
                    index=['SUB', 'SR'].index(st.session_state["cota"])
                )
